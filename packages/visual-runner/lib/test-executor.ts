...
export class TestExecutor {
  ...
  async executeActAssert(
    ...
    if (!this.context) {
      throw new Error(
        'Context not initialized - executeArrange must be called first'
      );
    }
      ...
      get: (target, prop) => {
        ...
        return (...args: unknown[]) => {
          ...
          const handleResult = (res: unknown) => {
            ...
            if (
              this.captureEnabled &&
              ['spawn', 'remove', 'commit', 'pause'].includes(prop as string)
            ) {
          ...
          if (typeof value === 'function') {
            ...
            return (...args: unknown[]) => {
              ...
              const handleResult = (res: unknown) => {
                ...
                if (
                  this.captureEnabled &&
                  ['spawn', 'remove', 'commit', 'pause'].includes(
                    prop as string
                  )
                ) {
