...
visual("Wall blocking - both terrain and entities block", {
  ...
  act: ({ spatial }) => {
    ...
    spatial.move(2, 3, 3, 3, GameLayers.ACTORS, (cell) =>
      isBlocked(cell, false),
    );
    spatial.commit();

    // Try to move into wall entity (should fail)
    spatial.move(2, 3, 4, 3, GameLayers.ACTORS, (cell) =>
      isBlocked(cell, false),
    );
    spatial.commit();
  },
...
visual("Actor blocking - actors block other actors", {
  ...
  act: ({ spatial }) => {
    ...
    spatial.move(10, 10, 11, 10, GameLayers.ACTORS, (cell) =>
      isBlocked(cell, false),
    );
    spatial.commit();
  },
...
visual("Empty floor blocking - voids block when setting enabled", {
  ...
  act: ({ spatial }) => {
    ...
    spatial.move(5, 8, 6, 8, GameLayers.ACTORS, (cell) =>
      isBlocked(cell, false),
    );
    spatial.commit();
...
visual("Collectibles non-blocking - player walks over items", {
  ...
  act: ({ spatial }) => {
    ...
    spatial.move(7, 5, 8, 5, GameLayers.ACTORS, (cell) =>
      isBlocked(cell, false),
    );
    spatial.commit();
  },
