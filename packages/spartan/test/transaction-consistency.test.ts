...
describe('Transaction Consistency', () => {
    it('defers all operations until commit', () => {
        ...
        expect(spatial.getEntityPosition(id1)).toEqual({x: 6, y: 6, layer: GameLayers.ACTORS});
        expect(spatial.getEntityPosition(id2)).toEqual({x: 6, y: 5, layer: GameLayers.ACTORS});
    });
    ...
    it('excludes pending removals from radius queries', () => {
        ...
        const all = spatial.getEntityIdsInRadius(5, 5, 2, { includePendingRemovals: true });
    ...
    it('processes operations in correct order: remove -> move -> spawn', () => {
        ...
        expect(spatial.getEntityPosition(id2)).toEqual({x: 6, y: 6, layer: GameLayers.ACTORS}); // Spawned
        expect(spatial.getEntityPosition(id3)).toEqual({x: 5, y: 5, layer: GameLayers.ACTORS}); // Spawned at cleared location
    });
    ...
    it('clearIntents cancels all pending operations', () => {
        ...
        expect(spatial.getEntityPosition(id1)).toEqual({x: 5, y: 5, layer: GameLayers.ACTORS});
    });
});
