...
function extractFileDescription(sourceFile: ts.SourceFile): string | null {
    ...
    if (ranges && ranges.length > 0) {
        ...
        const commentContent = fileText.substring(firstComment.pos, firstComment.end);
        ...
        const explicitTag = cleanLines.find(line => line.startsWith('@brief') || line.startsWith('@description'));
        if (explicitTag) {
            ...
            if (ts.isClassDeclaration(node)) {
                ...
                node.members.forEach(member => {
                    if (ts.isMethodDeclaration(member) && member.name && ts.isIdentifier(member.name)) {
                        const isPrivate = (member as any).modifiers?.some((m: any) => m.kind === ts.SyntaxKind.PrivateKeyword);
                        const isProtected = (member as any).modifiers?.some((m: any) => m.kind === ts.SyntaxKind.ProtectedKeyword);
    ...
    for (const item of items) {
        ...
        if (item.isDirectory()) {
            ...
            try {
                ...
                const sourceFile = ts.createSourceFile(item.name, content, ts.ScriptTarget.Latest, true);
