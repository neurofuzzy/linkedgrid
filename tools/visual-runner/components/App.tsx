...
import {
  ...
  type Snapshot,
  type TestResult,
  type VisualTestDefinition,
} from '../lib/test-executor.js';
...
export function App() {
  ...
  const snapshots = useMemo(() => {
    ...
    state.type,
    state.type === 'running' || state.type === 'completed'
      ? state.snapshots
      : null,
      ...
      <Box marginBottom={1}>
        ...
        {state.type !== 'selecting' && (
          <>
            <Text dimColor>
              {' '}
              - {state.testName} ({state.testIndex + 1}/{flatTests.length})
            </Text>
            {renderStatus()}
          </>
        )}
